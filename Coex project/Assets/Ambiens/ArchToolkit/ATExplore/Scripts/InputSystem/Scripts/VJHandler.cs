...
public class VJHandler : MonoBehaviour, IDragHandler, IPointerUpHandler, IPointerDownHandler
{
    ...
    public void OnDrag(PointerEventData ped)
    {
        ...
        if (position.sqrMagnitude <= 0.05f)
            return;

        float x = (jsContainer.rectTransform.pivot.x >= 0.5f) ? position.x * 2 + 1 : position.x * 2 - 1;
        float y = (jsContainer.rectTransform.pivot.y >= 0.5f) ? position.y * 2 + 1 : position.y * 2 - 1;
